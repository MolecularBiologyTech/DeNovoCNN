...
if [[ "$USE_CONDA" = true ]]; then
    ...
    # Copy pipeline script to installation directory
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [[ -f "$SCRIPT_DIR/run_denovocnn_pipeline.sh" ]]; then
        echo -e "${BLUE}Copying pipeline script to installation directory...${NC}"
        cp "$SCRIPT_DIR/run_denovocnn_pipeline.sh" "$INSTALL_DIR/"
        chmod +x "$INSTALL_DIR/run_denovocnn_pipeline.sh"
        echo -e "${GREEN}Pipeline script copied successfully${NC}"
    else
        echo -e "${YELLOW}Warning: Pipeline script not found in installer directory${NC}"
    fi
