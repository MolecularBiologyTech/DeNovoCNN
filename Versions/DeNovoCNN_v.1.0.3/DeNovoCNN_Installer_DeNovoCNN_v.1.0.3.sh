...
# Generate pipeline script to installation directory
echo -e "${BLUE}Generating pipeline script to installation directory...${NC}"
cat > "$INSTALL_DIR/run_denovocnn_pipeline.sh" << 'PIPELINE_EOF'
#!/bin/bash
...
load_install_config() {
    if [[ -f "$INSTALL_DIR/install_config.sh" ]]; then
        ...
        echo -e "${YELLOW}Ensure you are running this script from the DeNovoCNN installation directory.${NC}"
    fi
}
...
print_help() {
    ...
    echo "  --install-dir PATH      DeNovoCNN installation directory (default: current directory)"
    echo "  --help                  Show this help message"
    echo ""
    echo "Example:"
...
chmod +x "$INSTALL_DIR/run_denovocnn_pipeline.sh"
echo -e "${GREEN}Pipeline script generated successfully at: $INSTALL_DIR/run_denovocnn_pipeline.sh${NC}"
...
echo -e "2. Run analysis pipeline: cd $INSTALL_DIR && ./run_denovocnn_pipeline.sh --help"
echo ""
