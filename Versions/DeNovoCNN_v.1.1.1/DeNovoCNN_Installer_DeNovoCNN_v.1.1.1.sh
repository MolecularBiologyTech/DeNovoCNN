...
if [[ "$USE_CONDA" = true ]]; then
    ...
    if [[ ! -d "$INSTALL_DIR" ]] || [[ ! -f "$INSTALL_DIR/environment.yml" ]]; then
        if [[ -d "$INSTALL_DIR" ]]; then
            echo -e "${YELLOW}Installation directory exists but environment.yml not found. Re-cloning DeNovoCNN...${NC}"
        else
...
cat > "$INSTALL_DIR/0_README_DeNovoCNN_Workflow.md" << 'EOF'
...
cat > "$INSTALL_DIR/1_Define_data_specs.txt" << EOF
#!/bin/bash
...
cat > "$INSTALL_DIR/2_Run_analysis.sh" << 'PIPELINE_EOF'
#!/bin/bash
...
chmod +x "$INSTALL_DIR/2_Run_analysis.sh"
echo -e "${GREEN}Analysis script generated successfully at: $INSTALL_DIR/2_Run_analysis.sh${NC}"
...
echo -e "3. Run analysis: cd $INSTALL_DIR && ./2_Run_analysis.sh"
echo ""
