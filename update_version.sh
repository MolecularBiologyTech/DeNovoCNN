...
SOURCE_FILE="$VERSIONS_DIR/$CURRENT_VERSION/DeNovoCNN_Installer_${CURRENT_VERSION}.sh"
...
DEST_FILE="$NEW_DIR/DeNovoCNN_Installer_${NEW_VERSION}.sh"
...
# Generate commit message by comparing the two versions
echo ""
echo "Comparing $CURRENT_VERSION to $NEW_VERSION..."
echo ""

# Use git diff to compare the files
DIFF_OUTPUT=$(git diff --no-index "$VERSIONS_DIR/$CURRENT_VERSION/DeNovoCNN_Installer_${CURRENT_VERSION}.sh" "$DEST_FILE" || true)
